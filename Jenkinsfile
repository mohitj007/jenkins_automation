   ...
   stages {
      stage('Creating base jobs') {
          steps {
             ...
             cd $WORKSPACE; for i in `cat jobstobuild`; do $cliconnect list-jobs|grep -i $i; if [ $? == 0 ]; then echo \\\'Job $i already present\\\'; else $cliconnect create-job $i < `pwd`/$i.xml; fi; done
